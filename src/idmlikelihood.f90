...
if(c(i).eq.1) then
    ...
    if(gausspoint.eq.10) then
        call qgaussPL(c(i), t1(i), t2(i), t3(i), the01, the12, the02, res2, vet01, vet12, vet02, gamma)
    ...
    call susp(t3(i), the12, nz12, su12, ri12, zi12, gl12)
    if(gausspoint.eq.10) then
        call qgaussPL(c(i), t1(i), t3(i),t3(i), the01, the12, the02, res2, vet01, vet12, vet02, gamma)
    else if(gausspoint.eq.15) then
...
!========================          idmlLikelihood         ====================
      ...
      nva12,nva02,nva12dep,t00,t10,t20,t30,troncature0,likelihood_res)

! change KLB: add nva12dep for time-dep cov in 12
...
!---------- calcul de la vraisemblance ------------------
        ...
        do i=1,no0
                ...
                if(nva01.gt.0)then
                        do j=1,nva01
                                vet01 =vet01 +&
                                bh(npar0-nva01-nva12-nva02-nva12dep +j)*dble(ve01(i,j))
                        end do
                endif  
                ...
                if(nva02.gt.0)then
                        do j=1,nva02
                                vet02 =vet02 +&
                                bh(npar0-nva02-nva12-nva12dep+j)*dble(ve02(i,j))
                        end do
                endif
                ...
                if(nva12.gt.0)then
                        do j=1,nva12
                                vet12 =vet12 +&
                                bh(npar0-nva12-nva12dep+j)*dble(ve12(i,j))
                        end do
                endif
	  ...
	  print *, 'Cas 4 test 3', c(i), t1(i), t2(i), t3(i),,the01,the02,the12,vet01,vet02,vet12, gamma
  !  print *, 't1(', i, ') = ', 
  !  print *, 't2(', i, ') = ', t2(i)
  !  print *, 't3(', i, ') = ', t3(i)
  !  print *, 'the01 = ', the01
...
!================================  QGAUS : 1   ==========================
subroutine qgauss1(cas,a,b,c,the01,the02,the12,res,v01,v02,v12_ref,gamma)
        implicit none
         double precision a,b,c,the01(2),the02(2),the12(2)
         double precision dx,xm,xr,w(5),x(5),res,v01,v02,v12_ref, gamma, v12, v12dem
         double precision xx,f1,su01,ri01,ri12,f2,su12,su02,ri02
         double precision ri12_T,gl12_T,su12_T
...
!================================  QGAUS : CHEBYCHEV   ==========================
  	...
  	use commun,only:zi01,zi12,zi02,nz01,nz12,nz02,zi12_T
         ...
         integer::j,cas
         double precision::a,b,c,dx,xm,xr,res,v1,v2,v3, v2_ref, gamma
...
! ici ajouter calcul de SUSP en T 
	       call susp(c,the12,nz12,su12_T,ri12_T,zi12_T,gl12_T)

	       f1 = (su01**v1)*(su02**v3)*(su12_T**v2)*ri01*v1/(su12**v2)
               xx = xm-dx
               call susp(xx,the01,nz01,su01,ri01,zi01,gl01)
               call susp(xx,the02,nz02,su02,ri02,zi02,gl02)
               call susp(xx,the12,nz12,su12,ri12,zi12,gl12)
...
! ici ajouter calcul de SUSP en T 
	       call susp(c,the12,nz12,su12_T,ri12_T,zi12_T,gl12_T)
               ...
               f2 = (su01**v1)*(su02**v3)*(su12_T**v2)*ri01*v1/(su12**v2)
               res = res + w(j)*(f1+f2)
            end do
            res = res*xr
         else
...
! ici ajouter calcul de SUSP en T 
	       call susp(c,the12,nz12,su12_T,ri12_T,zi12_T,gl12_T)
               ...
               f1 = (su01**v1)*(su02**v3)*(su12_T**v2)*ri01*v1 * ri12_T * v2/(su12**v2)
...
! ici ajouter calcul de SUSP en T 
	       call susp(c,the12,nz12,su12_T,ri12_T,zi12_T,gl12_T)
               ...
               f2 = (su01**v1)*(su02**v3)*(su12_T**v2)*ri01*v1* ri12_T * v2/(su12**v2)
